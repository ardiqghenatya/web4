...
define('RP_WCDPD_VERSION', '1.0.16');
    ...
    class RP_WCDPD
    {
        ...
        public function __construct()
        {
            ...
            // Load frontend scripts/styles and hook into WooCommerce
                ...
                // Change prices and totals in cart (and mini cart)
                add_action('woocommerce_cart_loaded_from_session', array($this, 'apply_discounts'), 100);
                add_action('woocommerce_ajax_added_to_cart', array($this, 'apply_discounts'), 100);
        ...
        public function apply_discounts()
        {
            ...
            if ($this->cart_discount_to_apply = $this->discounts->get()) {
                ...
                add_action('woocommerce_before_calculate_totals', array($this, 'apply_fake_coupon'));
            }

            // Remove cart discounts (if previously applied but no longer valid)
            else {

                // Check if we don't have real coupon by the same name
                $coupon = new WC_Coupon($this->opt['settings']['cart_discount_title']);

                // Remove coupon if it does not exist but is applied to cart
                if (!$coupon->id && $woocommerce->cart->has_discount($this->opt['settings']['cart_discount_title'])) {
                    add_filter('woocommerce_coupons_enabled', array($this, 'woocommerce_enable_coupons'));
                    $this->remove_woocommerce_coupon($this->opt['settings']['cart_discount_title']);
        ...
        public function remove_woocommerce_coupon($coupon)
        {
            ...
            if (self::wc_version_gte('2.1')) {
                ...
                $coupon = apply_filters('woocommerce_coupon_code', $coupon);
        ...
        {
            if ($coupon_code == apply_filters('woocommerce_coupon_code', $this->opt['settings']['cart_discount_title'])) {
                $coupon = array(
                    'id'                            => 2147483647,
                    'type'                          => 'fixed_cart',
                    ...
                    'usage_count'                   => '',
                    'expiry_date'                   => '',
                    'apply_before_tax'              => ((isset($this->opt['settings']['apply_cart_discounts']) && $this->opt['settings']['apply_cart_discounts'] == 'incl_tax') ? 'no' : 'yes'),
                    'free_shipping'                 => 'no',
            ...
            $this->to_javascript = array(
                ...
                'hints' => array(
                    ...
                    'settings' => array(
                        ...
                        //'rp_wcdpd_settings_apply_cart_discounts_field' => __('', 'rp_wcdpd'),
        ...
        public function options_validate_fields($context, $input, $output)
        {
            ...
            if ($context == 'pricing') {
                ...
                $output['apply_cart_discounts'] = (isset($input['apply_cart_discounts']) && in_array($input['apply_cart_discounts'], array('excl_tax', 'incl_tax'))) ? $input['apply_cart_discounts'] : 'excl_tax';
