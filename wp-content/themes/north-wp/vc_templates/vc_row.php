...
if ( $enable_parallax ) {
	...
	$data = ' data-parallax-speed="' . $parallax_speed . '" data-parallax-direction="'.$parallax_direction.'"';
}

if ( $type !== 'in_container' ) {
	$output .= '<div id="'.$row_id.'" class="'.$type_class.$height.' '.$parallax.$video.$el_class.vc_shortcode_custom_css_class($css, ' ').'" '.$data.'>';
}

$output .= $bg_video;
$output .= '<div '.(( $type == 'in_container' ) ? ' id="'.$row_id.'"' : '').' class="'.$nopadding. (( $type !== 'in_container' ) ? 'row' : $css_class.$parallax.$el_class.$height.vc_shortcode_custom_css_class($css, ' ')) .'"'.$equal_height. (( $type != 'in_container' ) ? '' : $data) .'>';
